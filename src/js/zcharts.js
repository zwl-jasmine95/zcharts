...
function zcharts(params) {
    ...
    zcharts.initSvg = function (id,option) {
        ...
        opts.initSeries = option.series;   /*初始绘图数据*/
         ...
         * (2)绘制legend
         */
        startDraw(option.series);
        drawLegend(opts);


        function startDraw(s) {
            var series = $.extend(true,[],s);
            opts.newSeries = series;   /*绘图数据*/
            /**
             * 判断绘图数据类型
        ...
        function drawPie(option) {
            ...
            var width = option.width || defaultOpts.width,
                ...
                svg = option.svg || defaultOpts.svg,
                tooltip = option.tooltip || defaultOpts.tooltip,
                newSeries = option.newSeries || defaultOpts.newSeries;

            var pie = d3.layout.pie(); //构造一个新的默认的饼布局

            newSeries.map(function (d,i) {
                var data = [];
                d.data.map(function (v,j) {
                    data.push(v.value);
                });
        ...
        function drawLegend(option) {
            ...
            var svg = option.svg || defaultOpts.svg,
                ...
                initSeries = option.initSeries || defaultOpts.initSeries,
                newSeries = option.newSeries || defaultOpts.newSeries;

            var legendBox = svg.append('g')
                .attr('class','legendBox')
                .attr('transform','translate(' + padding.left + ',' + (height - padding.bottom/2) + ')');
            initSeries.map(function (d,i) {
                var legendGroup = legendBox.append('g')
                    .attr('class','legendGroup')
                    .attr('tag',1)   //是否显示标志位
                    .attr('color',d.color)
                    .attr('transform','translate(' + (i % 5) * 110 + ',' + Math.floor(i/5) * 23 + ')');
                ...
                if(d.type == 'bar'){
                    ...
                    var lineTag = legendGroup.append('g')
                        .attr('tag',1)    //是否显示标志位
                        .attr('class','lineTag')
                        .attr('stroke',d.color);
                    lineTag.append('line')
                ...
                legendBox.selectAll('.legendGroup')
                    .on('click',function (d,i) {
                        var obj = d3.select(this);
                        var color = d3.select(this).attr('color');
                        if(obj.attr("tag")==1){
                            obj.select('rect').attr("fill","#c0c0c0");
                            obj.select('.lineTag').attr("stroke","#c0c0c0");
                            obj.select('text').attr("fill","#c0c0c0");
                            obj.attr("tag",0);
                        }else{
                            obj.select('rect').attr("fill",color);
                            obj.select('.lineTag').attr("stroke",color);
                            obj.select('text').attr("fill","#000");
                            obj.attr("tag",1);
                        }
                        var data = [];
                        svg.selectAll(".legendGroup").each(function (v,j) {
                            if(d3.select(this).attr("tag")==1){
                                data.push(initSeries[j]);
                            }
                        });
                        svg.selectAll("svg>g:not(.legendBox)").remove();
                        console.log(data);
                        startDraw(data);

                    });
                //     .on('mouseover',function (d,i) {
                //         var rectGroup = d3.selectAll('.rectGroup')[0];
                //         rectGroup.map(function (v,j) {
                //             if(i != j){
                //                 $('.rectGroup').eq(j).attr('opacity',0.2);
                //             }
                //         });
                //     })
